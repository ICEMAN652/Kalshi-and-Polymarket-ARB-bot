...
export default function Dashboard() {
  ...
  // Derive a reliable reference price — prefer polymarket, fall back to kalshi
  const referencePrice = data.polymarket?.price_to_beat
    ?? data.polymarket?.current_price
    ?? data.kalshi?.current_price
    ?? 0
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Target: {data.polymarket?.slug ?? 'Loading...'}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-100 p-3 rounded-md">
                  ...
                  <div className="text-xl font-mono font-semibold">
                    ${(data.polymarket?.price_to_beat ?? referencePrice)?.toLocaleString() ?? 'N/A'}
                  </div>
                </div>
                <div className="bg-slate-100 p-3 rounded-md">
                  <div className="text-xs text-muted-foreground uppercase font-bold">Current Price</div>
                  <div className="text-xl font-mono font-semibold">
                    ${(data.polymarket?.current_price ?? data.kalshi?.current_price)?.toLocaleString() ?? 'N/A'}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-mono font-medium">${data.polymarket?.prices?.Up?.toFixed(3) ?? 'N/A'}</span>
                </div>
                ...
                <div className="flex justify-between items-center text-sm mt-2">
                  ...
                  <span className="font-mono font-medium">${data.polymarket?.prices?.Down?.toFixed(3) ?? 'N/A'}</span>
                </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Ticker: {data.kalshi?.event_ticker ?? 'Loading...'}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="bg-slate-100 p-3 rounded-md mb-4">
                <div className="text-xs text-muted-foreground uppercase font-bold">Current Price</div>
                <div className="text-xl font-mono font-semibold">
                  ${data.kalshi?.current_price?.toLocaleString() ?? 'N/A'}
                </div>
              </div>

              <div className="space-y-3 max-h-[200px] overflow-y-auto pr-2">
                {(data.kalshi?.markets ?? [])
                  .filter(m => Math.abs(m.strike - referencePrice) < 2500)
                  .map((m, i) => (
        ...
        <CardContent>
          {data.checks.length === 0 ? (
            <div className="text-center text-muted-foreground py-8">
              No arbitrage checks available — waiting for price data.
            </div>
          ) : (
            <Table>
              <TableHeader>
